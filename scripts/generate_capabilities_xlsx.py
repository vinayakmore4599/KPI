...
def component_rows() -> list[list]:
    return [
        ...
        [
            ...
            "G/R are examples. group_by, ignore_filters, also_emit. Request grain overlays selected_dimensions.",
            "cuts:\n  - { name: G, group_by: [], ignore_filters: [region], also_emit: [R] }\n  - { name: R, group_by: [region] }",
            "Global avg is recomputed, never mean of regional avgs.",
...
def all_operations_rows() -> list[list]:
    return [
        ...
        ["Aggregation", "first / last", "Identity fold after sort by time (or window).", "base_measures.bal: { sql: balance, agg: last }", "Used with over: last_n"],
        ...
        ["Time", "grain day/week/month/quarter/year", "Bucket of the time column. Fiscal calendar only for Q/Y.", "time: { column: event_month, grain: month, filter_code: reporting_month }", "parameters.time_grain overlay if declared"],
        ["Time", "point offset", "Same measure at calendar offset from anchor.", "previous_year_value: { of: sotif_value, op: point, offset: { years: 1 } }", ""],
        ["Time", "window trailing / leading", "Inclusive trailing N (or leading) periods as one number.", "value_3m: { of: sotif_value, op: window, trailing: { months: 3 }, inclusive: true }", ""],
        ["Time", "window range", "ytd, qtd, mtd, wtd, cumulative, full_month/quarter/year.", "value_qtd: { of: sotif_value, op: window, range: qtd }", "wtd needs day grain"],
