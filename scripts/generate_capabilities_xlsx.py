...
def preparation_rows() -> list[list]:
    ...
    return [
        ...
        [
            ...
            "agg: sum | avg | count | count_distinct | min | max | median | percentile | first | last | stddev | variance | mode.",
        ],
        [
        ...
        [
            ...
            "lag of trend/rank. lag of a row helper (no agg:). Rank a lagged measure instead of lagging a rank. Put offset: on the trend.",
            "Sources: bases and shiftable measures (point, window, fn, expr, hook, …).",
        ],
        [
            "Map — row mask (where:)",
...
def decision_rows() -> list[list]:
    return [
        ...
        [
            ...
            "op: percent_of_total (optional partition_by, versus_cut)",
...
def all_operations_rows() -> list[list]:
    return [
        ...
        ["Row helper", "where:", "Pandas row mask before fold. Also legal on a measure whose of: is a base (clone). Numeric ops coerce the compare column. ne excludes nulls (SQL-style).", "where: { column: mrr, op: gt, value: 0 }\nmeasures.late: { of: fact, op: point, where: { column: status, op: eq, value: LATE } }", "Ops: in eq ne gt gte lt lte between. between needs values: [lo, hi]. Not like/is_null. ignore_filters: on a measure skips context codes."],
        ...
        ["Aggregation", "stddev / variance / mode", "Non-additive sample stdev/var (ddof=1). One row → null. mode ties → smallest.", "base_measures.spread: { sql: amount, agg: stddev }", "Same path as median"],
